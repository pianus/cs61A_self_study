...
def balanced(m):
    ...
    if is_weight(m):
        ...
        if length(s1) * total_weight(end(s1)) != length(s2) * total_weight(end(s2)):
            ...
            return balanced(end(s1)) and balanced(end(s2))
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        temp, year = self.balance, 0

        while temp<amount:
            temp *= self.interest+1
            year += 1
        return year
...
class FreeChecking(Account):
    ...
    def withdraw(self,amount):
        self.withdrawals_numbs += 1
        if amount > self.balance:
            return 'Insufficient funds'
        elif self.withdrawals_numbs > FreeChecking.free_withdrawals:
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, passcode):
        nonlocal incorrect_numbs

        if incorrect_numbs >=3:
            lst = str(incorrect_password)
            return "Your account is locked. Attempts: "+ lst
        else:
            if passcode == password:
                ...
                incorrect_numbs +=1
                incorrect_password.append(passcode)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, password):
        if password in pass_lst:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, password)

    correctness = withdraw(0, old_password)
    if  type(correctness) != str:
        pass_lst.append(old_password)
        pass_lst.append(new_password)
        return joint
    else:
        return correctness
...
def check_par():
    ...
    r2 = interval(-3, 2) # Replace this line!
    return r1, r2

def multiple_references_explanation():
    return """The multiple reference problem arise because multiple evaluation
    interval of a single variable in one equation. This would result the samle
    interval adds and substitute with itself, which result that final interval
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) < extreme_point and upper_bound(x) > extreme_point:
...
def polynomial(x, c):
    ...
    def dir_poly(c):
        ...
        return dc
    f = lambda x: g(c,x)
    df = lambda x: g(dir_poly(c),x)
    ddf = lambda x: g(dir_poly(dir_poly(c)),x)

    num_steps = 20
    interval_range = upper_bound(x) - lower_bound(x)
    step_size = interval_range/20
    newton_start_points = [lower_bound(x)+ (k+0.5) * step_size for k in range(num_steps-1)]
    extremes = [find_zero(df,ddf,n) for n in newton_start_points]
    points = [n for n in extremes if n > lower_bound(x) and n < upper_bound(x)] + [lower_bound(x), upper_bound(x)]
    values = [g(c,n) for n in points]
