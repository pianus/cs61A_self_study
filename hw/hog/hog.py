...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def score_gain_string(score):
        if score == 1:
            string = "1 point!"
        elif score >= 1:
            string = "%i points!" % score
        return string

    def player_announce_string(who):
        return "That's the biggest gain yet for Player %i" % who

    def score_gain(score):
         return score - previous_score

    def highest(score0, score1, who = who, previous_high = previous_high, previous_score = previous_score):
        if who == 0:
            score = score0
        elif who == 1:
            score = score1

        if score_gain(score) > previous_high:
            print(score_gain_string(score_gain(score)),player_announce_string(who))
            previous_high = score_gain(score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    k, highest, num_of_dice =1, 0, 0
    while k <= 10:
        average = make_averaged(roll_dice)(k,dice)
        if average > highest:
            highest = average
            num_of_dice = k
        k += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if (is_swap(baconed_score, opponent_score) and opponent_score > baconed_score
        or free_bacon(opponent_score) >= margin):
        return 0
    else:
        return num_rolls# Replace this statement
    # END PROBLEM 11
