...
class Ant(Insect):
    ...
    def can_contain(self, other):
        ...
        "*** YOUR CODE HERE ***"
        if self.container:
            if not self.ant and not other.container:
                return True
        return False
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while place.entrance and place.entrance != hive:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        def ninja_attact(place):
            bees = list(place.bees)
            for bee in bees:
                bee.reduce_armor(self.damage)

        place = self.place
        ninja_attact(place) #ninja first attacts bees in its place
        #then ninja attacts bees in the front places
        while place.entrance and place.entrance is not colony.hive:
            place = place.entrance
            ninja_attact(place)
        #the ninja attacks bees in the back places
        place = self.place
        while place.exit:
            place = place.exit
            ninja_attact(place)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        def double_damage(queen_ant, ant):
            if ant not in queen_ant.buffed_ants:
                ant.damage *= 2
            ...
            while place.exit:
                ...
                if place.ant:
                    ...
                    if place.ant.container:
                        if place.ant.ant:
                            double_damage(self, place.ant.ant)
...
def apply_effect(effect, bee, duration):
    ...
    original_action = copy(bee.action)
    def new_action(colony):
        nonlocal duration
        if duration >= 1:
            bee.action = effect(original_action)
            duration -= 1
        else:
